            ...
            for schema in observed_schemas:
                ...
                for db_entity in db_entities:
                    ...
                    if all(db_entity.identity != le.identity for le in local_entities):
                        # Sequences are frequently referenced by column defaults.
                        # Use CASCADE so autogenerated upgrades can drop them reliably.
                        op = DropOp(
                            db_entity,
                            cascade=(db_entity.__class__.__name__ == "PGSequence"),
                        )
                        drop_ops.append(op)
                        logger.info(
